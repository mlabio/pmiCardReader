...
class CardReader(object):
	...
	def __init__(self, name, GPIO_7, GPIO_8, TIMEOUT):
	...
	def addBitToTag(self, gpio_id, val):
		...
		if gpio_id == self.GPIO_7:
			self.tag += "0"
		elif gpio_id == self.GPIO_8:
			self.tag += "1"	

	def registerReader(self, edge = 'falling', pull_up_down=RPIO.PUD_UP):
		RPIO.setup(self.GPIO_7, RPIO.IN)
		RPIO.setup(self.GPIO_8, RPIO.IN)
		RPIO.add_interrupt_callback(self.GPIO_7, self.addBitToTag, edge = edge, pull_up_down = pull_up_down)
		RPIO.add_interrupt_callback(self.GPIO_8, self.addBitToTag, edge = edge, pull_up_down = pull_up_down)
	...
	def removeReader(self):
		RPIO.del_interrupt_callback(self.GPIO_7)
		RPIO.del_interrupt_callback(self.GPIO_8)
